...
const Index: React.FC = (props) => {
  ...
  useEffect(() => {
    if (strategyPreviewData && strategyPreviewData['charts']) {
      strategyChartData = strategyPreviewData['charts'].slice(0, 5);
      setEchartOption({
        tooltip: {
          trigger: 'axis',
          axisPointer: {
  ...
  return (
    <Card
      title={t('workflow.overview.exec.scheduler.overview')}
      bordered={true}
      ...
      <Row style={{ padding: '0 24px' }}>
        <Col flex="160px">
          <Row>
            <Col>
              <Statistic
                title={t('workflow.overview.exec.scheduler.successed')}
                value={strategyPreviewData['successJob']}
                style={{ marginBottom: 24 }}
              />
            </Col>
          </Row>
          <Col span={24}>
            <Statistic title={t('workflow.overview.exec.scheduler.failed')} value={strategyPreviewData['failJob']} />
          </Col>
        </Col>
        <Col flex="1">
          {echartOption['series'] ? (
