...
const Page = ({ location }) => {
  ...
  const dataSourceId = location.query.dataSourceId * 1;
  ...
  useEffect(() => {
    if (schemaRegistry?.registryId && dataSourceId) {
      getDataFields();
    }
  }, [schemaRegistry?.registryId, dataSourceId]);

  const isShowDataSet = useMemo(
    () => ['kafka', 'collect'].includes(dataSourceDetail.dbType as string),
    [dataSourceDetail],
  );
  const filterSchemaRegistries = useMemo(
    () =>
      _.filter(
        schemaRegistries,
        (item) => item?.datasourceId === dataSourceId && item?.code.indexOf(dataSetKeyword) > -1,
      ),
    [dataSetKeyword, schemaRegistries, dataSourceId],
  );
  ...
  const getDataSourceData = async () => {
    const result = await detailRequest.runAsync(dataSourceId);

    await schemaRegistryRequest.runAsync(dataSourceId);
  ...
  const getDataBaseNode = () => {
    const database = dataSourceDetail.database;
    const schemaTable = dataSourceDetail.schemaTable;
  ...
  return (
    <div className={styles.detailPage}>
      <PageHeader dataSource={dataSourceDetail} pageType={undefined} onTabChange={undefined} />
